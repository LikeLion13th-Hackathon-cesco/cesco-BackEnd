...
public class ComplaintReportUpload { // 고소장 업로드 서비스 로직
  ...
  {
    ComplaintReport savedReport = uploadFile(pathName, file);

    return ComplaintResponse.builder()
        .complaintReportId(savedReport.getComplaintReportId())
        .userId(savedReport.getUser().getUserId())
        .fraudRegisterReportId(savedReport.getFraudRegisterReport().getFraudRegisterReportId())
  ...
  @Transactional
  public ComplaintReport uploadFile(PathName pathName, MultipartFile file) {
    ...
    // 등기부 등본 엔티티 생성
    ComplaintReport complaintReport = ComplaintReport.builder()
    ...
    log.info("DB 저장 성공: reportId={}, fileName={}", savedReport.getComplaintReportId(), originalFilename);
  ...
  public Boolean deleteReport(Long reportId) {
    ...
    // DB에서 등기부등본 조회
    ComplaintReport report = complaintReportRepository.findById(reportId)
    ...
    try {
      ...
      // DB에서 레코드 삭제
      complaintReportRepository.delete(report);
