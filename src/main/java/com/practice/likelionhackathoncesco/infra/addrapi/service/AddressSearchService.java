...
public class AddressSearchService{
  ...
  public List<AddressSearchResponse> searchAddress(AddressSearchRequest addressSearchRequest){
    ...
    String encodedKeyword = URLEncoder.encode(addressSearchRequest.getKeyword(), StandardCharsets.UTF_8);

    // 기본 요청 값 지정
    if (addressSearchRequest.getConfmKey() == null || addressSearchRequest.getConfmKey()
        .isEmpty()) {
    ...
    if (addressSearchRequest.getResultType() == null || addressSearchRequest.getResultType()
        .isEmpty()) {
    ...
    try{
      ...
      if (addrResults.isArray()) {
        for (JsonNode node : addrResults) {
          AddressSearchResponse addressSearchResponse = AddressSearchResponse.builder()
