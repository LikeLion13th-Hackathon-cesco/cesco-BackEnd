...
public class GptComplaintService {
  ...
  public String callGptAPI(List<Map<String, String>> prompts, String fraudRegisterReportId) {
    ...
    try {
      log.info(
          "[GptComplaintService] GPT API 요청 시도 : fraudRegisterReportId={}", fraudRegisterReportId);
      // POST 요청
      ...
      if (response.getStatusCode() != HttpStatus.OK) { // 응답코드 200일때만 응답 꺼내기
        log.error(
            "[GptComplaintService] GPT API 응답 실패 : fraudRegisterReportId={}, httpStatus={}",
            fraudRegisterReportId,
      ...
      choices = objectMapper.convertValue(responseBody.get("choices"), new TypeReference<>() {});

      if (choices == null || choices.isEmpty()) {
        log.warn(
            "[GptComplaintService] GPT API 응답 성공했으나 choices 변수 null : fraudRegisterReportId={}",
            fraudRegisterReportId);
      ...
      log.info(
          "[GptComplaintService] 응답 성공 : fraudRegisterReportId={}, content={}",
          fraudRegisterReportId,
          content);
      ...
      log.error(
          "[GptComplaintService] GPT API 클라이언트 오류: fraudRegisterReportId={}, {}",
          fraudRegisterReportId,
      ...
      log.error(
          "[GptComplaintService] GPT API 타임아웃 또는 접근 실패: fraudRegisterReportId={}, {}",
          fraudRegisterReportId,
          e.getMessage());
      ...
      log.error(
          "[GptComplaintService] GPT API 호출 중 예외 발생: fraudRegisterReportId={}, {}",
          fraudRegisterReportId,
          e.getMessage());
      throw new CustomException(GptErrorCode.GPT_API_CALL_FAILED);
    }
